# Harder, shorter, sharper, forward: A comparison of women's and men's elite football gameplay (2020-2025)

Data, code, materials for "[Harder, shorter, sharper, forward: A comparison of women's and men's elite football gameplay (2020-2025)]([url](https://arxiv.org/abs/2506.22119))"

_DATA_

Although the data used in this study are not publicly available, an equivalent surrogate dataset (same provider and same format) can be found in this [GitHub repository]([url](https://github.com/statsbomb/open-data)).

_CODE_

[Pitch passing network
](./pitch_passing_network/)

[KPI statistics](./kpis/)
